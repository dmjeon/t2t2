 ...
 * AADC PoC 검증 화면
 *
 * 브라우저가 직접 두드리는 것은 자기 WEB 하나뿐이다. 반대편 DC 는
 * nginx 가 /peer/ 로 대신 물어본다. CORS 를 피하려는 것도 있지만,
...
const LAYERS = [
  ...
  { id: 'peer',  name: '반대편 DC', url: '/peer/health/deep',
    desc: 'GSLB 가 넘길 수 있는 곳이 살아 있는가. ★ WEB 끼리는 라우팅 경로가 없다 — '
        + '닿지 못한 것은 "반대편이 죽었다"가 아니라 "여기서는 볼 수 없다"이다.',
    detail: (b) => b.status === 'ok'
      ? `${b.was_host} (${b.dc}) v${b.app_version}`
      : (b.check === 'deep' ? `${b.reason || 'fail'}` : '도달 못 함 — 여기서는 판정 불가'),
    // 앱이 답했을 때만 판정한다. 502/504/타임아웃은 경로가 없다는 뜻이고,
    // 그건 GSLB 가 밖에서 볼 일이지 이 서버가 보고할 일이 아니다.
    grade: (r) => {
      const b = r.body || {};
      if (b.check !== 'deep') return 'na';
      return b.status === 'ok' ? 'ok' : 'fail';
    } },
];
...
function renderPathMap(infoR, layers) {
  ...
  const peerR = layers.peer, dbR = layers.db;
  ...
  if (web && !info && isDenied(infoR)) {
    ...
    if (direct) {
      fail.push(me.eDirect, me.was);
    } else if (st === 504) {
      cur.push(me.eWl, me.l4, me.eFadc, me.fadc);
      fail.push(me.eLw, me.was);
    } else {
      fail.push(me.eWl, me.l4);
    }
    sFail.push(`WEB → WAS ${st ? 'HTTP ' + st : '연결 실패'}`);
    sCur.push('여기서 끊김');
  } else if (info) {
    if (direct) {
      cur.push(me.eDirect, wasSide.was);
      sCur.push('다이렉트(L4 우회)');
    } else {
      cur.push(me.eWl, me.l4, me.eFadc, me.fadc);
    ...
    sCur.push((info.was_host || 'WAS') + ' / ' + wasDc);
   ...
   *
   * backup(교차) 멤버는 평시에 판정하지 않는다 (2026-09-24). 실제로 넘어갔을 때
   *    위의 cross 분기가 FAILOVER 로 칠하는 것으로 확인한다.
